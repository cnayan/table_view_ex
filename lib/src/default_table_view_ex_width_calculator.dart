...
class DefaultTableViewExWidthCalculator extends TableViewExWidthCalculator {
  static final DefaultTableViewExWidthCalculator _instance = DefaultTableViewExWidthCalculator._internal();
  ...
  List<double> calculateColumnWidths(double viewportWidth, List<TableViewExColumnConfig> columnDefinitions) {
    ...
    final List<double> calculatedColumnWidths = Iterable.generate(columnSpans.length, (_) => 0.0).toList();
    ...
    if (remainingCount > 0) {
      final remainingWidth = (viewportWidth - totalFixedAndFractionalWidth) / remainingCount;
